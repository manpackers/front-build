    ...
    [
      {
        ...
        options: {
          patterns: options.injectStyle.map(value => path.join(process.cwd(), options.root, value))
        }
      }
    ]
  )
...
const generator = (env = 'development', ic, config) => {
  ...
  const isLibrary = config.output && config.output.library
  ...
  return {
    ...
    resolve: {
      extensions: ['.js', '.json', '.hbs', '.ts', '.tsx', '.vue', '.jsx', '.ejs', '.jade'],
      alias: { '@': resolveRoot }
    },
    ...
    module: {
      ...
      ].concat(ic.isEslint ? [createEslintLoader(/\.(mj|t|j)s$/, { fix: ic.isEslintFix })] : [])
    },
    ...
    plugins: [
      ...
      ...(ic.isStylelint
        ? [new StylelintPlugin({ fix: ic.isStylelintFix, files: ['**/*.s?(a|c)ss', '**/*.l?(e|c)ss'] })]
        : []),
    ...
    ].concat(ic.isCssExtract ? [new MiniCssExtractPlugin({ filename: 'css/[name].[hash:13].css' })] : [])
  }
}

process.noDeprecation = true
module.exports = { createCssLoader, generator, createURILoader, createEslintLoader }
