...
module.exports = async (ic, config) => {
  const builder = new Builder(ic, config)
  const complie = webpack(builder.webpackConfig)
  const spinner = ora('  Project version start to build……\n\n').start()
  const output = builder.webpackConfig.output.path
  let stats = null
  ...
  ic.isZip && (await file.zip(output, `v${packageConfig.version}`))
  // eslint-disable-next-line no-console
